 ...
 * conjunto de reglas e intenciones.
 */
...
/* -------------------------------------------------------------- */
/* Prefabs: composiciones por palabra clave                       */
/* -------------------------------------------------------------- */
...
function buildHouse(api, origin, color) {
  ...
  api.addShape({ shape: 'cone', color: 0xe2604f, position: [origin[0], origin[1] + 1.9, origin[2]], scale: [1.7, 1.2, 1.7], name: 'Tejado' });
...
function buildSnowman(api, origin) {
  ...
  api.addShape({ shape: 'cone', color: 0xff8a3d, position: [origin[0], origin[1] + 2.05, origin[2] + 0.5], scale: [0.15, 0.5, 0.15], name: 'Nariz' });
...
function buildRocket(api, origin, color) {
  ...
  [[-0.4, 0], [0.4, 0], [0, -0.4], [0, 0.4]].forEach(([dx, dz], i) => {
    api.addShape({ shape: 'cone', color: 0xff8a3d, position: [origin[0] + dx, origin[1] + 0.25, origin[2] + dz], scale: [0.15, 0.4, 0.15], name: `Aleta ${i + 1}` });
  });
...
export function interpret(rawText, api) {
  ...
  // Si no es un prefab, intentar añadir figuras simples
  const shape = findShape(text);
  if (shape) {
    const color = findColor(text) ?? 0xe2604f;
    const material = findMaterial(text) ?? 'standard';
    const nums = findNumbers(text);
    const position = nums.length >= 3 ? [nums[0], nums[1], nums[2]] : [0, 0, 0];
